...
| 3 | **커스텀 도메인은 `bf16` 권장** (`fp16`은 도면/요소에서 수치 불안정 → 깨진 출력; CORD 원본은 `fp16=True`로 동작) | 0점/깨진 출력 |
...
> ⚠️ **task 이름 ≠ 어떤 JSON 키 이름** (충돌 방지)
> task 토큰도, 필드 키도 **둘 다 `<s_이름>` 토큰**이 된다. 그래서 이름이 같으면 **같은 토큰**이 되어 충돌한다 —
> task 토큰을 떼는 단계(`replace`)에서 **같은 이름의 필드 여는 태그까지 지워져** token2json 파싱이 깨진다(→ 점수 0).
> 그래서 task와 키를 **다른 이름**으로 둔다:
> - 도면: task `<s_drawing>` ↔ 최상위 필드 `title_block` (≠ drawing) ✅
> - CORD: task `<s_cord-v2>` ↔ 필드 `menu` / `total` … ✅
> - 요소: task `<s_element>` ↔ 필드 `dimension` / `gdt` … ✅
