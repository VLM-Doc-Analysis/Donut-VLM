...
## 0-2. 고DPI 비순환 재평가 (2026-06-29) — ✅ 검수 GT

§0-1 의 순환 문제를 제거. held-out val 100개를 **고DPI 크롭을 직접 판독(이미지 대조)** 하여 GT 를 교정·확정한 뒤, 그 val 을 학습에서 제외하고 재학습(`train_dpi_ab --val-ids`)·재평가.

| 구성 | val GT 출처 | val n | Field-F1 | charsim | exact |
|:--|:--|--:|--:|--:|--:|
| §0-1 순환 | 모델 pre-fill(자기 자신) | ~98 | 0.835 | 0.919 | 83.5% |
| **§0-2 비순환** | **이미지 대조 검수 GT** | **98** | **0.816** | **0.934** | **81.6%** |

**검수 내역(100개 중):** 모델 초안 대비 **교정 18건 + 판독불가 제외 2건**, 나머지 80건은 초안이 정확.
- 숫자 오독 4 (예: `Ø15.68→15.58`, `PCD55→555`, `N1→11`)
- 각도 `°` 누락 4 (`120→12°`, `60→60°`, `180→18°`, `6→6°`)
- 회전 크롭 오독 3 (`⊥.2→Ra6.3`, `Ra8→Ra6.3`, `→3×53.33`)
- Datum 오독 1 (`D→A`), 표기 정규화 6 (`x→×`·공백), 판독불가 2 (`#9`,`#89`)

> ✅ **이것이 첫 정직한(비순환) 고DPI 수치다.** 비순환 0.816 이 순환 0.835 와 **거의 같다** — val 초안의 80%가 실제로 맞았기 때문(고DPI 크롭이 선명해 모델이 잘 읽음). 즉 §0-1 의 순환 편향은 작았고, **고DPI 학습 성능은 실재**한다.
