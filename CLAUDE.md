...
- **요소 Donut 파인튜닝** — 아키텍처의 파인튜닝 메커니즘(json2token, 특수 토큰 등록, Teacher Forcing, bf16 등)을
  **요소 크롭 단위**(도면 전체 아님, 정렬된 요소 크롭 1개)로 적용. **JSON 구조화 방식이 다른 두 노트북**이 있다:
  ...
  - **핵심 차이**: `parse_to_schema` 의 위치 — flat=**모델 출력 사후처리** ↔ paper=**학습 타깃 전처리**.
    공통: task token `<s_element>`, 기호(Ø,⊥,±,°)는 토크나이저 실제 토큰 등록(U+XXXX 폐기),
    데이터는 `detection/cvat_to_donut.py` 로 CVAT export 에서 생성(`{"<type>":"<value>"}`).
  - **선택 기준**: 소량 데이터(~1,975장·GD&T 180)에선 **flat 이 안전**(중첩 태그 직접 생성은 데이터 부족 시 깨진
    `<s_…>` 태그 위험). 데이터(특히 GD&T·Hole)가 충분해지면 **paper** 가 사후 정규식 한계를 넘어선다.
