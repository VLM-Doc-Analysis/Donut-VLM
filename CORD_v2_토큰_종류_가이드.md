...
> 이 필드 토큰들은 학습 때 라벨에 등장한 키를 토크나이저에 **등록**(`add_tokens` 또는 `add_special_tokens`)해 디코더가 생성할 수 있게 한 것이다. — 공개 CORD-v2 체크포인트는 **`add_tokens`** 로 등록돼 `all_special_tokens` 엔 안 보이고 `get_added_vocab()` 로 조회된다.
